...
/** Returns the given sheet data in a structured way using the startRow as the one with the name of the key */
export function structureArrayDataByColumn(
	inputData: string[][],
	keyRow: number,
	dataStartRow: number,
): IDataObject[] {
	...
	return structureData(inputData, dataStartRow, keys);
}
...
export function encodeRange(range: string): string {
	if (range.includes('!')) {
